...
import type { SponsorMethod } from "../../hooks";

import { motion } from "motion/react";

interface SponsorMethodInfoProps {
  methods: SponsorMethod[];
}

const SponsorMethodInfo = ({ methods }: SponsorMethodInfoProps) => {
        ...
        {methods.map((method, index) => (
          ...
          >
            ...
            <div className="flex flex-col gap-[0.4rem] max-mobile:gap-[0.2rem]">
              <span className="text-[1.5rem] font-[600] text-black max-mobile:text-[1.4rem]">{method.label}</span>
              <span className="text-[1.4rem] text-dark_gray break-keep max-mobile:text-[1.3rem]">{method.value}</span>
            </div>
          </li>
        ))}
      </ul>
...
export default SponsorMethodInfo;
